...
public class NotificationService {
    ...
    public void createNotification(String text, NotificationType type, Order order) {
        ...
        switch (type) {
            case WAITER_COOK:
                messagingTemplate.convertAndSend("/topic/cook", notification);
                break;
            case WAITER_BARMAN:
                messagingTemplate.convertAndSend("/topic/barman", notification);
                break;
            case COOK_WAITER:
            case BARMAN_WAITER:
                messagingTemplate.convertAndSendToUser(order.getWaiter().getUsername(), "/queue/waiter", notification);
                break;
            default:
                break;
        }
